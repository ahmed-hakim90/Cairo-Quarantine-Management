...
import { DestinationCountryRequirementsPicker } from "@/components/international/DestinationCountryRequirementsPicker";
...
export default async function InternationalTravelerPage({
  ...
  const [vaccinesByCategory, offices, destinationCountries] = await Promise.all([
    listVaccinesByCategoryForPublic(),
    listOffices(),
    listDestinationCountriesForPublic(),
  ]);
      ...
      <ScrollReveal>
        <section className="mx-auto max-w-6xl px-4 py-10">
          <div className="rounded-lg border border-gov-gray-200 bg-white p-6 shadow-sm md:p-8">
            ...
            <DestinationCountryRequirementsPicker
              countries={destinationCountries}
              intro={p.destinationVaccinesIntro}
              labels={p.countryRequirements}
            />
          </div>
        </section>
      </ScrollReveal>
