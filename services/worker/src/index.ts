...
const tickMs =
  Number.parseInt(process.env.WORKER_TICK_MS ?? "120000", 10) || 120_000;
const retentionEveryTicks =
  Number.parseInt(process.env.WORKER_RETENTION_EVERY_TICKS ?? "6", 10) || 6;
...
console.info(
  `[worker] starting (tick every ${tickMs}ms, retention every ${retentionEveryTicks} ticks)`,
);
void tick();
